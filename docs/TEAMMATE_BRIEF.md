# Tier 1 + Tier 2 — build brief (the Tier 2 owner)
...
the Tier 3 owner's. Where the two disagree, `INTERFACE.md` wins.
...
|---|---|---|
| **Sep 6** | Phototransistor package checked | You know whether PT204-6B works or you are on the fallback. One message to the Tier 3 owner either way. |
...
Message the Tier 3 owner. Specifically, message immediately if:
...
Everything the Tier 3 owner is building runs against mocks and needs no hardware from you, so a delay on your
